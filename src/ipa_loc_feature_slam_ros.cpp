...
//       sub_truth_odom_ = n_.advertise<nav_msgs::Odometry>( "/visualization_truth_odom" , 10 );
   ...
   void update(){
      ...
      // Ensure initalized coordinate frame and time reference. Is initalized with the first pushed data.
      if( !initialized )
      {
	 initialized = prog_.getInitStatus(); 
      }
      else 
      {
         prog_.update( config_ );
	 
	  // Broadcast transform from /base_link to /slam_map 
	 ...
	 pub_feature_markers_.publish( prog_.getFeatureMarkersCandidates() );
...
//        if(msg_featr->source_id != "rfid" && ( !msg_featr->lines.empty() ) )
       {
	 prog_.pushNewData( msg_featr );
       }
   }   
   ...
   bool service_callback_write_best_map( ipa_navigation_msgs::success::Request &req , ipa_navigation_msgs::success::Response &res )
   {
      ...
      std::string temp = config_.best_map_file_name;
      ROS_INFO("hello");
      try
      {
